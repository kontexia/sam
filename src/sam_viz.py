...
def plot_pors(pors, name=None):
    ...
    for x in range(len(pors)):
        ...
        ema_error.append(por['ema_error'])
        ema_stdev.append(pow(por['ema_variance'], 0.5))

        if por['anomaly']:
            anomalies.append(por['bmu_distance'])
        else:
            anomalies.append(None)
        if por['motif']:
            motifs.append(por['bmu_distance'])
        else:
            motifs.append(None)
    ...
    compression_ratio = round(por["nos_neurons"] / len(pors), 2)
    fig.update_layout(width=1200, height=1200,
                      title=dict(text=f'{por["sam"]} nos neurons: {por["nos_neurons"]} Nos Raw_data: {len(pors)} Ratio: {compression_ratio}'))
    fig.show()


def plot_pors_v2(pors, name=None):
    ...
    fig = go.Figure(data=[nos_neurons_scatter, similarity_scatter])
