...
class Neurons_v1(object):
    ...
    def to_dict(self, decode: bool = True) -> dict:
        ...
        d_neurons = {'learning_rate_decay_factor': self.learning_rate_decay_factor,
    ...
    def feed_forward(self,
                     sdr: SDR,
                     activation_group_keys: set = None,
        ...
        for key in sdr.encoding:
            if ((activation_group_keys is None or key[0] in activation_group_keys) and
                    (activation_temporal_keys is None or key[1] in activation_temporal_keys) and
                    (activation_enc_keys is None or key[2] in activation_enc_keys)):

                for bit in sdr.encoding[key]:

                    normalisation_factor += sdr.encoding[key][bit]

                    # if this key and bit have been mapped then process
                    #
                    if key in self.bit_to_neuron and bit in self.bit_to_neuron[key]:
                        for neuron_key in self.bit_to_neuron[key][bit]:
                            ...
                            if neuron_key not in activated_neurons:
                                ...
                                activated_neurons[neuron_key] = (sdr.encoding[key][bit] * self.neuron_to_bit[neuron_key]['sdr'].encoding[key][bit])
                            else:
                                activated_neurons[neuron_key] += (sdr.encoding[key][bit] * self.neuron_to_bit[neuron_key]['sdr'].encoding[key][bit])
    ...
    def update_bit_to_neuron(self, neuron_key: cython.int):
        ...
        for key in neuron_sdr.encoding:
            if key not in self.bit_to_neuron:
                self.bit_to_neuron[key] = {}

            # remove the connection if bit not in neuron_sdr - it may have been pruned
            #
            for bit in self.bit_to_neuron[key]:
                if bit not in neuron_sdr.encoding[key]:
                    self.bit_to_neuron[key][bit].discard(neuron_key)

            # make sure all bits in neuron_sdr have a connection to this neuron
            #
            for bit in neuron_sdr.encoding[key]:
                if bit not in self.bit_to_neuron[key]:
                    self.bit_to_neuron[key][bit] = {neuron_key}
                else:
                    self.bit_to_neuron[key][bit].add(neuron_key)
    ...
    def update_communities(self, activated_neurons: list):
        ...
        for idx_1 in range(len(activated_neurons)):
            for idx_2 in range(len(activated_neurons)):
                if idx_1 != idx_2:
                    self.neuron_to_bit[activated_neurons[idx_1]['neuron_key']]['community'].add(activated_neurons[idx_2]['neuron_key'])
